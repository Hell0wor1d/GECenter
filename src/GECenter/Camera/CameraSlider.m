...
@interface CameraSlider () {
    CGFloat gap;
    CGFloat inverseGap;
}

@property (nonatomic, assign) SliderDirection direction;//方向（横或竖）
...
{
    return [self initWithFrame:frame direction:SliderDirectionHorizonal];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat startLineX = (_direction == SliderDirectionHorizonal ? gap : (self.frame.size.width - _lineWidth) / 2);
    CGFloat startLineY = (_direction == SliderDirectionHorizonal ? (self.frame.size.height - _lineWidth) / 2 : gap);
    
    CGFloat endLineX = (_direction == SliderDirectionHorizonal ? self.frame.size.width - gap : (self.frame.size.width - _lineWidth) / 2);
    CGFloat endLineY = (_direction == SliderDirectionHorizonal ? (self.frame.size.height - _lineWidth) / 2 : self.frame.size.height- gap);
    ...
    CGContextSetLineWidth(context, _lineWidth);//线的宽度
    CGFloat slidedLineX = (_direction == SliderDirectionHorizonal ? MAX(gap, (_scaleNum * self.frame.size.width - gap)) : startLineX);
    CGFloat slidedLineY = (_direction == SliderDirectionHorizonal ? startLineY : MAX(gap, (_scaleNum * self.frame.size.height - gap)));
    CGContextMoveToPoint(context, startLineX, startLineY);//起点
    ...
    CGFloat circleX = (_direction == SliderDirectionHorizonal ? MAX(_circleRadius + penWidth, slidedLineX - penWidth - inverseGap) : startLineX);
    CGFloat circleY = (_direction == SliderDirectionHorizonal ? startLineY : MAX(_circleRadius + penWidth, slidedLineY - penWidth - inverseGap));
...
- (void)updateTouchPoint:(NSSet*)touches {
    ...
    self.scaleNum = (_direction == SliderDirectionHorizonal ? touchPoint.x : touchPoint.y) / (_direction == SliderDirectionHorizonal ? self.frame.size.width : self.frame.size.height);
}
