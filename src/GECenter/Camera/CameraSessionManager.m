...
#import "Common/UIImage+Resize.h"
...
- (void)addSession {
    ...
    //    设置质量
    //    注意如果设置为最高图片质量会导致 内存警告造成程序闪退的情况，解决方案是使用低质量图片或者对拍摄后的高质量图片进行缩放处理
    ...
    _session.sessionPreset = AVCaptureSessionPresetHigh;//AVCaptureSessionPresetPhoto;
}
...
- (void)takePicture:(DidCapturePhotoBlock)block {
    ...
    [_stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        DLog(@"origin Image:%@", [NSValue valueWithCGSize:image.size]);
        // [SCCommon saveImageToPhotoAlbum:image];
        
        // 如果要进行正方形的裁剪，需要恢复下面的代码
        // CGFloat squareLength = APP_SIZE.width;
        ...
        // DLog(@"scaled Image:%@", [NSValue valueWithCGSize:scaledImage.size]);
        ...
        // DLog(@"crop Frame:%@", [NSValue valueWithCGRect:cropFrame]);
        // UIImage *croppedImage = [scaledImage croppedImage:cropFrame];
        // DLog(@"cropped Image:%@", [NSValue valueWithCGSize:croppedImage.size]);
        ...
        if (orientation != UIDeviceOrientationPortrait) {
            // UIDeviceOrientationFaceUp 朝上平躺
            // UIDeviceOrientationFaceDown 朝下平躺
            // UIDeviceOrientationUnknown 無法判斷目前Device的方向，有可能是斜置
            CGFloat degree = 0;
...
- (void)switchFlashMode:(UIButton*)sender {
    ...
    if ([device hasFlash]) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示信息" message:@"您的设备没有闪光灯功能" delegate:nil cancelButtonTitle:@"好的" otherButtonTitles: nil];
        [alert show];
    }
...
- (void)switchGrid:(BOOL)toShow {
    ...
    //    CGFloat headHeight = 0;//_previewLayer.bounds.size.height;// - APP_SIZE.width;
    //    CGFloat squareLength = APP_SIZE.width;
    ...
    for (int i = 1; i< 9; i++) {
        ...
        if (i<= 3) {//绘制竖线
            frame = CGRectMake(i  * APP_SIZE.width / 3, 44, 1, APP_SIZE.height - 88);
        }
        else{ //绘制横线
            frame = CGRectMake(0, (i-3) * APP_SIZE.height / 6 , APP_SIZE.width, 1);
        }
