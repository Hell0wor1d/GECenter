...
#define SWITCH_SHOW_DEFAULT_IMAGE_FOR_NONE_CAMERA   1   //没有拍照功能的设备，是否给一张默认图片

//height
#define CAMERA_TOPVIEW_HEIGHT   44  //标题视图高度
#define CAMERA_MENU_VIEW_HEIGH  44  //底部菜单栏高度
...
@property (nonatomic, strong) CameraSlider *slider;
...
#pragma marks
    ...
    //session manager
    CameraSessionManager *manager = [[CameraSessionManager alloc] init];
    
    //AvcaptureManager
    if (CGRectEqualToRect(_previewRect, CGRectZero)) {
        //预览视图大小设置
        self.previewRect = CGRectMake(0, 0, APP_SIZE.width, APP_SIZE.height);// + CAMERA_TOPVIEW_HEIGHT);
    }
    [manager configureWithParentLayer:self.view previewRect:_previewRect];
    self.captureManager = manager;
    
    [self addTopViewWithText:@"请对准题目进行拍照"]; //添加顶部标题
    
    //[self addbottomContainerView];
    
    [self addCameraMenuView]; //添加底部菜单栏
    
    [self addFocusView]; //添加聚焦
    
    [self addCameraCover];
    
    [self addPinchGesture];//添加伸缩尽头 手势
...
- (void)addTopViewWithText:(NSString*)text {
    if (!_topContainerView) {
        ...
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, APP_SIZE.width, topFrame.size.height)];
...
//    CGRect bottomFrame = CGRectMake(0, bottomY, APP_SIZE.width, APP_SIZE.height - bottomY);
//
//    UIView *view = [[UIView alloc] initWithFrame:bottomFrame];
//    view.backgroundColor = bottomContainerView_UP_COLOR;//[UIColor clearColor];//;
...
- (void)addCameraMenuView {
    ...
    [self buildButton:CGRectMake((APP_SIZE.width - cameraBtnLength) / 2, (APP_SIZE.height - downH - cameraBtnLength) , cameraBtnLength, cameraBtnLength)
         normalImgStr:@"shot.png"
      highlightImgStr:@"shot_h.png"
       selectedImgStr:@""
               action:@selector(takePictureBtnPressed:)
           parentView:self.view];//_bottomContainerView
    
    
    //拍照的菜单栏view（屏幕高度大于480的，此view在上面，其他情况在下面）
    CGFloat menuViewY = (isHigherThaniPhone4_SC ? DEVICE_SIZE.height - CAMERA_MENU_VIEW_HEIGH : 0);
...
- (void)addMenuViewButtons {
    ...
    [CameraCommon drawALineWithFrame:CGRectMake(eachW, 0, 1, CAMERA_MENU_VIEW_HEIGH) andColor:rgba_SC(102, 102, 102, 1.0000) inLayer:_cameraMenuView.layer];
    
    
    //屏幕高度大于480的，后退按钮放在_cameraMenuView；小于480的，放在_bottomContainerView
    for (int i = 0; i < actionArr.count; i++) {
        
        CGFloat theH = CAMERA_MENU_VIEW_HEIGH;//(!isHigherThaniPhone4_SC && i == 0 ?  _bottomContainerView.frame.size.height : CAMERA_MENU_VIEW_HEIGH);
        UIView *parent = _cameraMenuView;//(!isHigherThaniPhone4_SC && i == 0 ? _bottomContainerView : _cameraMenuView);
...
- (void)addCameraCover {
    ...
    UIView *downView = [[UIView alloc] initWithFrame:CGRectMake(0, APP_SIZE.height, APP_SIZE.width, 0)];
...
- (void)showCameraCover:(BOOL)toShow {
    ...
    [UIView animateWithDuration:0.38f animations:^{
        ...
        upFrame.size.height = (toShow ? APP_SIZE.height / 2: 0);//+ CAMERA_TOPVIEW_HEIGHT : 0);
        _doneCameraUpView.frame = upFrame;
        
        CGRect downFrame = _doneCameraDownView.frame;
        downFrame.origin.y = (toShow ? APP_SIZE.height / 2 : APP_SIZE.height);// self.view.frame.origin.y);
        downFrame.size.height = (toShow ? APP_SIZE.height / 2 : 0);
...
- (void)addPinchGesture {
    ...
    CameraSlider *slider = [[CameraSlider alloc] initWithFrame:CGRectMake(_previewRect.size.width - width, (_previewRect.size.height + CAMERA_MENU_VIEW_HEIGH - height) / 2, width, height) direction:SliderDirectionVertical];
    slider.alpha = 0.f;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:ADJUSTINT_FOCUS]) {
        ...
        //        DLog(@"Is adjusting focus? %@", isAdjustingFocus ? @"YES" : @"NO" );
        //        DLog(@"Change dictionary: %@", change);
        if (!isAdjustingFocus) {
            alphaTimes = -1;
        }
    }
...
- (void)takePictureBtnPressed:(UIButton*)sender {
    ...
    [_captureManager takePicture:^(UIImage *stillImage) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [CameraCommon saveImageToPhotoAlbum:stillImage];//存至本机
        });
        ...
        //your code 0
        CameraNavigationController *nav = (CameraNavigationController*)weakSelf_SC.navigationController;
        if ([nav.cameraNaigationDelegate respondsToSelector:@selector(didTakePicture:image:)]) {
            [nav.cameraNaigationDelegate didTakePicture:nav image:stillImage];
        }
        //or your code 1
...
- (void)handlePinch:(UIPinchGestureRecognizer*)gesture {
    ...
    if (_slider) {
        ...
        [_slider setValue:_captureManager.scaleNum shouldCallBack:NO];
