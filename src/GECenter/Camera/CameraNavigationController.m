...
- (void)dismissModalViewControllerAnimated:(BOOL)animated {
    ...
    if ([self.cameraNaigationDelegate respondsToSelector:@selector(willDismissNavigationController:)]) {
        shouldToDismiss = [self.cameraNaigationDelegate willDismissNavigationController:self];
    }
    if (shouldToDismiss) {
...
- (void)showCameraWithParentController:(UIViewController*)parentController {
    CameraController *con = [[CameraController alloc] init];
