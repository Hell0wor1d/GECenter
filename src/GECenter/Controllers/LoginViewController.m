...
- (void)addControls
{
    // Add Icon
    if (_icon == nil) {
        _icon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"login-logo"]];
    ...
    if (_usernameText == nil) {
        ...
        _usernameText.placeholder = NSLocalizedString(@"用户名 / 邮箱", @"NUX First Walkthrough Page 2 Username Placeholder");
    ...
    if (_passwordText == nil) {
        ...
        _passwordText.placeholder = NSLocalizedString(@"密码", nil);
    ...
    if (_userIsDotCom) {
        signInTitle = NSLocalizedString(@"登陆", nil);
    } else {
        signInTitle = NSLocalizedString(@"添加站点", nil);
    }
    ...
    if (_skipToCreateAccount == nil) {
        ...
        _skipToCreateAccount.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
        ...
        [_skipToCreateAccount setTitle:NSLocalizedString(@"注册账户", nil) forState:UIControlStateNormal];
    ...
    if (_forgotPassword == nil) {
        ...
        _forgotPassword.autoresizingMask = UIViewAutoresizingFlexibleRightMargin |UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleBottomMargin;
...
- (void)layoutControls
{
    ...
    CGSize fsize = [_forgotPassword.titleLabel.text sizeWithAttributes:@{NSFontAttributeName:_forgotPassword.titleLabel.font}];
    x = (viewWidth - GeneralWalkthroughButtonWidth )/ 2.0 ;//+ fsize.width / 2.0;//CGRectGetMaxX(_signInButton.frame);//(viewWidth - GeneralWalkthroughButtonWidth) / 2.0;
    y = CGRectGetMaxY(_signInButton.frame) + 0.5 * GeneralWalkthroughStandardOffset;
    CGFloat forgotPasswordHeight = fsize.height;
    _forgotPassword.frame = CGRectIntegral(CGRectMake(x, y, GeneralWalkthroughButtonWidth / 2.0, forgotPasswordHeight));
    
    // Layout Skip to Create Account Button
    //x = GeneralWalkthroughStandardOffset;
    x =  (viewWidth - GeneralWalkthroughButtonWidth)/2.0 + GeneralWalkthroughButtonWidth / 2.0;
    CGFloat createAccountHeight = [_skipToCreateAccount.titleLabel.text sizeWithAttributes:@{NSFontAttributeName:_skipToCreateAccount.titleLabel.font}].height;
    //y = CGRectGetMaxY(_forgotPassword.frame) + 0.5 * GeneralWalkthroughStandardOffset;//viewHeight - GeneralWalkthroughStandardOffset - GeneralWalkthroughSecondaryButtonHeight;
    _skipToCreateAccount.frame = CGRectIntegral(CGRectMake(x, y, GeneralWalkthroughButtonWidth /2.0, createAccountHeight));//GeneralWalkthroughSecondaryButtonHeight));
...
{
    return YES;//[[_usernameText.text trim] length] != 0;
}

- (BOOL)isPasswordFilled
{
    return YES;//[[_passwordText.text trim] length] != 0;
}

- (BOOL)isSiteUrlFilled
{
    return YES;//[[_siteUrlText.text trim] length] != 0;
}
